...
const Editor = () => {
  ...
  const [searchedComponentArray, setSearchedComponentArray] = useState<
    Component[]
  >([]);
  ...
  const takeSnapshot = async (
    ...
    console.log(
      `[editor] snapshot taken with metadata name: ${name}, type ${type}`,
    );
        ...
        params.forEach((param) => {
          if (param.startsWith("frombp=")) {
            ...
            if (frombp === "true") {
              ...
              console.log(
                "[editor] frombp is true, setting fromBlueprint to true",
              );
            } else {
              setFromBlueprint(false);
              console.log(
                "[editor] frombp is false, setting fromBlueprint to false",
              );
            }
          }
          ...
          if (param.startsWith("designname=") && !hasRun.current) {
            ...
            logOnDebug(
              "editor",
              `decompressed data (attempt): ${decompressed}`,
            );
            ...
            if (!decompressed) {
              logOnDebug(
                "editor",
                "Decompression returned null, trying direct parse",
              );
              try {
                ...
                if (Array.isArray(directParse)) {
                  logOnDebug(
                    "editor",
                    "Direct parse successful, using parsed data",
                  );
                ...
                decompressed =
                  LZString.decompressFromEncodedURIComponent(decoded);
                logOnDebug(
                  "editor",
                  `Decompressed from decoded data: ${decompressed}`,
                );
              }
            }
            ...
            if (decompressed) {
              ...
              try {
                ...
                if (typeof decompressed === "string") {
                  console.log(
                    "[editor] Decompressed data is a string, parsing it",
                  );
                  parsedData = JSON.parse(decompressed);
                  console.log(
                    "[editor] typeof parsedData: ",
                    typeof parsedData,
                  );
                  if (typeof parsedData === "string") {
                    parsedData = JSON.parse(parsedData);
                    console.debug(
                      "[editor] Parsed data is a string, re-parsed to JSON",
                      parsedData,
                    );
                  }
                } else {
                ...
                if (!Array.isArray(parsedData)) {
                  console.error(
                    "[editor] Parsed data is not an array, converting to array",
                  );
                ...
                if (renderer.current) {
                  ...
                  console.log(
                    `[editor] performing final checks, isarray: ${Array.isArray(finalData)}, finalData: `,
                    finalData,
                  );
                  console.log("[editor] fromBlueprint state:", fromBlueprint);
                  if (id.includes("frombp=true") || fromBlueprint) {
                    console.log(
                      "[editor] fromBlueprint is true, performing direct assignment",
                    );
                    renderer.current.logicDisplay!.components = Array.isArray(
                      parsedData,
                    )
                      ? parsedData
                      : [parsedData];
                    ...
                    console.log(
                      "[editor] fromBlueprint is false, using importJSON",
                    );
                  ...
                  if (renderer.current.logicDisplay!.components.length === 0) {
                    console.error(
                      "[editor] No components found in the design, initializing with an empty array",
                    );
                  }
                }
              } catch (error) {
                console.error(
                  "[editor] Failed to parse decompressed data:",
                  error,
                );
                console.warn(
                  "[editor] Attempting to continue with empty array",
                );
              }
            }
            setLoading(false);
        ...
        <div className={styles["export-dialog"]}>
          {exportPage == "main" && (
            <>
              ...
              <div>
                <div
                  className={`${styles["export-option"]} ${styles.special}`}
                  onClick={() => {
                    if (renderer.current && renderer.current.logicDisplay) {
                      ...
                      navigator.clipboard.writeText(
                        window.location.href.replace("/action=new;", url),
                      );
                    }
                ...
                >
                  ...
                  &nbsp;&nbsp;
                  <p>
                    {getLocaleKey("editor.main.header.shareModal.copyLink")}
                  </p>
                </div>
                  ...
                  {id.split(";")[1] && (
                    <div className={styles["export-options-container"]}>
                      <div className={styles["export-option-sub"]}>
                        <div
                          className={styles["export-option-sub-button"]}
                          onClick={() => setExportPage("embed")}
                        >
                          <img src={Embed} width={24} />
                        </div>
                        <span>
                          {getLocaleKey(
                            "editor.main.header.shareModal.embedToSite",
                          )}
                        </span>
                      </div>
                    </div>
